...
foreach ($switches as $switch) {
    ...
    if ($port_to_ifindex !== false) {
        ...
        // 3. Get FDB table (MAC to Bridge Port + VLAN)
        ...
        if ($fdb_table) {
            ...
            foreach ($fdb_table as $oid => $val) {
                ...
                if ($mac_addr && $port_name) {
                    $stmt = $db->prepare("INSERT INTO switch_port_map (mac_addr, switch_id, port_name, vlan_id) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE port_name = VALUES(port_name), vlan_id = VALUES(vlan_id), updated_at = CURRENT_TIMESTAMP");
                    $stmt->execute([$mac_addr, $switch['id'], $port_name, $vlan_id]);
                    $discovered_count++;
                }
            }
